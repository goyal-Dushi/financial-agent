...
def build_agent(user_id: str, request_id: str) -> Agent:
    ...
    return Agent(
        ...
        instructions=(
            ...
            "When image evidence is required, call the read_payment_image tool. Keep outputs "
            "faithful to the dataset; never fabricate rows."
        ),
        model=build_model("data_generator"),
        model_settings=SETTINGS.model_settings(),
        tools=[*tools, vision.as_tool(
            tool_name="read_payment_image",
            tool_description="Extract financial facts (amounts, pay slips) from the user's supporting images via OCR.",
        )],
